...
public class Modify_2 : MonoBehaviour
{
	...
	void calcMidPoint(){
		...
		foreach (Face f in listOfFaces) {
			...
			case "Share X Axis":
				...
				//GameObject tempXGO = Instantiate (caps, new Vector3 (avgX, avgY, avgZ), Quaternion.identity);
				if (fMode) {
			...
			case "Share Z Axis":
				/*
				temp1 = f.vList [0].transform.position;
				temp2 = f.vList [1].transform.position;
				avgX = Mathf.Min (temp1.x, temp2.x) + Mathf.Abs ((temp1.x - temp2.x) / 2);
				avgY = Mathf.Min (temp1.y, temp2.y) + Mathf.Abs ((temp1.y - temp2.y) / 2);
